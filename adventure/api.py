...
def initialize(request):
  ...
  ##### Check for any possible rooms by uuid and get coord of player
  ##### if none exist, generate world via the World_generation function. save.() all the rooms. Return data
  world_array = player.findWorld()
  if len(world_array) > 1:
    return JsonResponse({'uuid': uuid, 'name':player.user.username, 'title':room.title, 'description':room.description}, safe=True)
  else:
    World_Generator(player)
    return JsonResponse({})
...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name':player.user.username, 'title':nextRoom.title, 'description':nextRoom.description, 'error_msg':""}, safe=True)
    else:
        players = room.playerNames(player_id)
        return JsonResponse({'name':player.user.username, 'title':room.title, 'description':room.description, 'error_msg':"You cannot move that way."}, safe=True)
